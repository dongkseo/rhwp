---
name: rhwp-cli
description: rhwp CLI 바이너리로 HWP/HWPX 파일을 분석·내보내기·디버깅합니다. SVG/PNG/텍스트/Markdown 내보내기, 페이지네이션·조판부호·render tree 덤프, IR 비교(HWPX↔HWP), HWPX→HWP 저장 계약 분석을 적절한 명령으로 수행합니다. HWP/HWPX/HML→PDF 변환 요청은 이 스킬이 아니라 hwp 스킬을 반드시 읽고 처리합니다. 트리거 — 사용자가 ".hwp/.hwpx 파일을 SVG/PNG/텍스트로 내보내", "페이지네이션/조판부호/구조 덤프", "두 파일 IR 비교", "render tree 추출", "레이아웃/간격/겹침 버그 디버깅", "HWPX→HWP 저장 차이 분석", "rhwp <명령>" 등을 요청할 때. 전체 명령 레퍼런스는 mydocs/manual/cli_commands.md.
---
...
`.hwp`/`.hwpx`/`.hml` 입력을 PDF로 변환하는 사용자 요청은 이 스킬이 아니라 `.claude/skills/hwp/`
스킬로 처리한다. PDF 변환은 bundled WASM 경로가 기본이며 native CLI fallback을 쓰지 않는다.
...
- `export-png`은 `--features native-skia` 의존 — release 바이너리에 포함됨.
...
| "PDF로" | hwp 스킬의 `scripts/export_pdf.mjs` |

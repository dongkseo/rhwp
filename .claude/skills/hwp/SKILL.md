...
필요 없다. 2.4MB, 설치 몇 초, 차트용 `@resvg/resvg-js` 도 함께 딸려온다.

```bash
npm i https://github.com/dongkseo/rhwp/releases/download/v0.7.19-pkg/dongkseo-rhwp-core-0.7.19.tgz
```

> `npm i github:dongkseo/rhwp` (저장소 clone) 는 쓰지 마라 — 691MB 를 받아 6분 걸린다.
> 위 릴리즈 tarball 은 `pkg/` 산출물만 담는다.

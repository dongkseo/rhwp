...
이 스킬은 rhwp 를 WebAssembly 로 쓴다. 정상 배포된 스킬은 `skills/hwp/pkg/`에
`rhwp.js`와 `rhwp_bg.wasm`을 포함하므로 HWP 열기·편집·PDF 변환에 npm/Rust/Docker가 필요 없다.
...
`scripts/hwp/loader.mjs` 는 `RHWP_PKG`, 스킬 내장 `pkg/`, 저장소 루트 `pkg/`, 설치된 `@rhwp/core`
...
| `npm i github:dongkseo/rhwp` 로 설치 | 691MB clone·6분이다 — 정상 배포된 스킬의 내장 `pkg/` 또는 `RHWP_PKG`를 써라 |
...
- WASM API 전체 명세: `skills/hwp/pkg/rhwp.d.ts` (스킬 내장), `pkg/rhwp.d.ts` (빌드 후 생성), 또는 `rhwp-studio/public/rhwp.d.ts` (커밋본)

---
name: hwp
description: "MUST use/read this skill for any request to convert a .hwp/.hwpx/.hml file to PDF. Use this skill any time a HWP document (한글 문서) is the primary input or output: open, read, edit, fix, create, extract text/tables/structure, or convert HWP/HWPX/HML to PDF using rhwp WASM PDF export. Trigger especially when the user references a .hwp file by name or path — even casually (like \"내려받은 한글 파일\", \"the hwp in my downloads\") — and wants something done to it or produced from it. Do NOT trigger for general PDF work unrelated to HWP/HWPX/HML input, Word/Excel deliverables, HTML reports, layout debugging, IR diffing (use the rhwp-cli skill), or converting exam papers into HWPX (use the rhwp-exam-ingest skill)."
...
`/opt/rhwp/fonts/NotoSansKR-Regular.ttf`를 바이트로 등록한다. 폰트 family가 렌더 fallback의
`Noto Sans KR`과 정확히 일치해야 한다. 별도 환경은 `RHWP_FONT_FILES`에 TTF/OTF/TTC 경로를 지정한다.
...
일부 환경(`--no-optional`, 특정 CI)에서 차트가 안 되면 래스터라이저만 따로:
...
`scripts/hwp/loader.mjs` 는 `RHWP_PKG`, Docker baked `/opt/rhwp/pkg`, 저장소 루트 `pkg/`, 설치된 `@rhwp/core`
순서로 찾는다. 설치 위치가 특이하면 `RHWP_PKG=/path/to/pkg` 로 지정한다.

## HWP/HWPX → PDF

`.hwp`/`.hwpx`/`.hml` 입력을 PDF로 변환하는 요청은 **반드시 이 hwp 스킬을 읽고 처리한다.**
`rhwp-cli` 스킬이나 host native CLI fallback으로 넘기지 않는다.

PDF 산출물이 필요하면 **HWP를 LibreOffice/soffice나 host native CLI에 맡기지 말고 rhwp WASM PDF export를
...
- `rhwp` native CLI fallback과 `SVG → PNG → PIL/reportlab image PDF`는 자동 fallback으로 쓰지 않는다.
...
| 내보내기 | `exportHwp(): Uint8Array`, `exportHwpx(): Uint8Array`, `exportPdf(textAsPaths): Uint8Array`, `exportPagePdf(page, textAsPaths): Uint8Array` |
...
| `npm i github:dongkseo/rhwp` 로 설치 | 691MB clone·6분이다 — Docker baked `/opt/rhwp/pkg` 또는 `RHWP_PKG`를 써라 |
...
- WASM API 전체 명세: Docker baked `/opt/rhwp/pkg/rhwp.d.ts`, `pkg/rhwp.d.ts` (빌드 후 생성), 또는 `rhwp-studio/public/rhwp.d.ts` (커밋본)

---
name: hwp
description: "MUST use/read this skill for every .hwp/.hwpx to PDF conversion request. Also use it any time a HWP document (한글 문서) is the primary input or output: open, read, edit, fix, create, or extract text/tables/structure. PDF conversion uses the Docker-baked upstream rhwp native CLI; HWP editing uses rhwp WASM. Trigger especially when the user references a .hwp file by name or path, even casually. Do NOT trigger for general PDF work unrelated to HWP/HWPX input, Word/Excel deliverables, HTML reports, layout debugging, IR diffing (use the rhwp-cli skill), or converting exam papers into HWPX (use the rhwp-exam-ingest skill)."
...
HWP 열기·편집·저장은 rhwp WASM을 사용한다. HWP/HWPX → PDF 변환은 Docker image에 baked 된
`/usr/local/bin/rhwp` native CLI를 사용한다. Rust는 Docker build stage에만 필요하며 sandbox
runtime에는 Rust 툴체인이 없다. 정상 image는 `fonts-noto-cjk`와 rhwp CLI를 포함한다.

```bash
npm i https://github.com/dongkseo/rhwp/releases/download/v0.7.18-pkg/dongkseo-rhwp-core-0.7.18.tgz
```

> `npm i github:dongkseo/rhwp` (저장소 clone) 는 쓰지 마라 — 691MB 를 받아 6분 걸린다.
> 위 릴리즈 tarball 은 `pkg/` 만 담아 2.4MB 다.
...
`scripts/hwp/loader.mjs`는 편집용 WASM 패키지를 찾는다. PDF wrapper는 `RHWP_BIN`, PATH의
`rhwp`, 저장소의 `target/release/rhwp` 순서로 native CLI를 찾는다.

## HWP/HWPX → PDF

`.hwp`/`.hwpx` 입력을 PDF로 변환하는 요청은 반드시 이 hwp 스킬을 읽고
`scripts/export_pdf.mjs`를 실행한다. runtime에서 `cargo build`하거나 WASM PDF로 우회하지 않는다.
...
| `npm i github:dongkseo/rhwp` 로 설치 | 691MB clone·6분이다 — 릴리즈 tarball URL 을 써라 (2.4MB·수초) |
| runtime에서 CLI를 빌드하거나 WASM PDF로 우회 | Docker-baked `/usr/local/bin/rhwp`를 `scripts/export_pdf.mjs`로 호출 |

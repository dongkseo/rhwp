...
차트만 예외적으로 **CLI**를 쓴다. 미리보기 래스터화에 `resvg`가 필요한데 그건 네이티브
전용이라 WASM에서 못 쓴다 (`export-png`도 같은 제약).
...
cat > chart.json <<'EOF'
{
  ...
  "widthMm": 150, "heightMm": 90,            // 개체 크기 (기본 150x90)
  "xMm": 30, "yMm": 60                       // 용지 기준 위치 (생략 시 좌상단)
}
EOF

cargo build --release --features native-skia   # 최초 1회 — resvg 포함
./target/release/rhwp insert-chart input.hwp --data chart.json -o output/with_chart.hwp
```

**한컴에서 편집 가능한 진짜 차트**로 들어간다 (더블클릭 → 차트 편집기 → 데이터 수정).
그림이 아니다. rhwp 안에서도 데이터가 살아있어 `renderPageSvg`로 그려진다.

WASM 코드 경로와 섞어 쓸 때는 **저장 → CLI → 재열기** 순서로 왕복한다:

```js
await saveHwp(doc, 'output/step1.hwp');
// $ rhwp insert-chart output/step1.hwp --data chart.json -o output/step2.hwp
const doc2 = await openHwp('output/step2.hwp');   // 컨트롤 인덱스 재탐색 필요
```

> 차트는 OLE 개체다. 한컴이 개체를 차트로 인식하는 근거는 OLE 내부 CFB 의
> **Root CLSID** (`{4C3DA137-DC90-47B9-9BED-59DAE352A280}`)다. 이게 없으면 편집이
> 안 되고 그림처럼 박힌다 — rhwp 는 이미 넣어주므로 신경 쓸 필요 없으나,
> 차트가 그림처럼 보인다면 이걸 먼저 의심하라.
...
| 차트 | CLI `rhwp insert-chart` 전용 (resvg 네이티브 제약) |
...
| 차트를 WASM 으로 만들려 시도 | resvg 가 네이티브 전용 — `rhwp insert-chart` 사용 |

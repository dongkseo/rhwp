...
글상자(TextBox) 안 텍스트도 같은 cell path 로 읽힌다. 다만 `getTextBoxControlIndex()`가
주는 인덱스로는 접근되지 않으므로, `findTextTargets()`처럼 컨트롤 인덱스를 훑어야 한다.
...
문단을 편집하면 rhwp 는 **그 아래 전부의 vpos 를 자기 레이아웃 엔진으로 재계산**한다
(`reflow_paragraph` → `recalculate_section_vpos(para_idx)` → `paginate`).
원본 파일에 든 한컴의 LINE_SEG 값이 그 지점부터 버려진다.

대부분은 문제없다 — 실문서 12개로 시험했을 때 11개는 편집 후에도 페이지 수가 그대로였다
(94쪽 문서 포함). 다만 rhwp 레이아웃이 한컴과 어긋나는 문서에서는 편집 위치에 따라 쪽수가 튄다:

```
한글 양식 샘플 (3).hwp — 탭이 많은 단축키 일람표, 원본 8쪽
  para=  0 편집 → 12쪽 (+4)     ← 문서 전체가 재계산됨
  para=100 편집 → 11쪽 (+3)
  para=250 편집 →  9쪽 (+1)
  para=304 편집 →  8쪽 ✅       ← 아래에 재계산할 게 없음
```

**편집 후 `pageCount()`를 원본과 비교하라.** 달라졌다면 레이아웃이 재계산된 것이고,
`exportHwpVerify()`는 이를 잡지 못한다 (편집 후 값끼리 비교하므로 12↔12 로 통과한다).
